...
public class ModelLoader {
    ...
    private static AnimMeshData processBones(AIMesh aiMesh, List<Bone> boneList) {
        ...
        for (int i = 0; i < numBones; i++) {
            ...
            for (int j = 0; j < numWeights; j++) {
                ...
                VertexWeight vw = new VertexWeight(bone.boneId(), aiWeight.mVertexId(),
                        aiWeight.mWeight());
                List<VertexWeight> vertexWeightList = weightSet.computeIfAbsent(vw.vertexId(), k -> new ArrayList<>());
        ...
        return new AnimMeshData(Utilities.listFloatToArray(weights), Utilities.listIntToArray(boneIds));
    }
